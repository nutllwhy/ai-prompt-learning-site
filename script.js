...
const casesData = {
    wenchuang: {
        name: '文创类',
        icon: '🎨',
        desc: '兼具收藏感、知识密度和二次创作空间，适合文旅号、知识号、出版社、展览物料和文创周边的原型生产。',
        cases: [
            {
                id: 1,
                title: "手绘城市美食地图",
    ...
    yingshi: {
        name: '影视类',
        icon: '🎬',
        desc: '体现电影感和提案感，把分镜、设定、棚拍、宣传板整合成完整的成片方案。',
        cases: [
            {
                id: 12,
                title: "史诗概念电影海报生成器",
    ...
    kuagip: {
        name: '跨IP类',
        icon: '🎮',
        desc: '让角色、界面、游戏语法和平台风格看起来像原本就属于同一个宇宙。',
        cases: [
            {
                id: 22,
    ...
    pinpai: {
        name: '品牌联动类',
        icon: '🏷️',
        desc: '完整模拟广告、直播、电商详情页、菜单、杂志、官网等真实商业载体。',
        cases: [
            {
                id: 32,
                title: "2x2 日本广告网格",
    ...
    egao: {
        name: '恶搞类',
        icon: '😂',
        desc: '把"梗"做成完整视觉成品，只要文字可读、画面够真、反差够大，就极易形成转发。',
        cases: [
            {
                id: 43,
                title: "宋朝社交媒体动态",
    ...
    dianshang: {
        name: '电商类',
        icon: '🛒',
        desc: '覆盖主图、促销海报、详情页、场景图、模特图、包装设计、食品摄影、直播间、小红书封面、跨境广告等电商全场景。',
        cases: [
            {
                id: 57,
                title: "白底主图：手持挂烫机",
...
// 生成案例卡片 HTML
function createCaseCard(caseItem, categoryName, categoryIcon) {
    return '<div class="case-card" data-category="' + categoryName + '">' +
        '<img src="' + caseItem.image + '" alt="' + caseItem.title + '" class="case-image" loading="lazy" onclick="openImageViewer(this.src)">' +
        '<div class="case-content">' +
        '<div class="case-category-tag">' + categoryIcon + ' ' + categoryName + '</div>' +
        ...
        '<button class="copy-btn" onclick="copyPrompt(this.parentElement.parentElement.querySelector(\'.case-prompt\').textContent)">复制提示词</button>' +
        '</div></div></div>';
}
...
function renderCases() {
    ...
    var html = '';
    var categories = Object.keys(casesData);
    
    // 根据筛选条件收集案例
    var filteredCases = [];
    
    categories.forEach(function(categoryKey) {
        var category = casesData[categoryKey];
        ...
        category.cases.forEach(function(caseItem) {
            ...
            if (currentSearch) {
                ...
                var titleMatch = caseItem.title.toLowerCase().indexOf(searchLower) !== -1;
                var promptMatch = caseItem.prompt.toLowerCase().indexOf(searchLower) !== -1;
    ...
    if (filteredCases.length === 0) {
        html = '<div class="no-results"><h3>未找到匹配的案例</h3><p>请尝试其他关键词或筛选条件</p></div>';
    } else {
        html = '<div class="cases-grid">';
        filteredCases.forEach(function(item) {
            html += createCaseCard(item.case, item.categoryName, item.categoryIcon);
        });
        html += '</div>';
    }
...
// 侧边栏控制
var sidebar, menuBtn, closeSidebar;

// 返回顶部
var backToTop;

// 导航高亮
var sections, navLinks, observer;
